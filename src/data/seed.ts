...
const createLiveVisits = (tenant: TenantSlug): Visit[] => {
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    return {
      ...
      facilityId: facility.id,
      status: 'checked_in',
      source: index % 3 === 0 ? 'planned' as const : 'spontaneous' as const,
      intent,
      plannedArrivalAt: index % 3 === 0 ? checkedInAt : undefined,
      originalPlannedArrivalAt: index % 3 === 0 ? checkedInAt : undefined,
      checkedInAt,
      expectedDurationMinutes: duration,
      expectedEndAt: addMinutes(checkedInAt, duration),
      autoCloseAt: addMinutes(checkedInAt, duration + university.autoCloseGraceMinutes),
