...
import { addSyntheticVisit, checkOutSyntheticVisit, delayNextPlannedVisit, moveSyntheticVisit } from '../services/demoOperations';
...
export function DemoControlsPage() {
  ...
  const hasPlannedVisit = state.visits.some((visit) => visit.status === 'planned' && visit.plannedArrivalAt);
  const hasSyntheticActiveVisit = state.visits.some((visit) => visit.status === 'checked_in' && visit.userId.startsWith(`${tenant}_synthetic`));
  ...
  return <div className="page-stack demo-page"><ManagementPageHeader eyebrow="Competition control room" title="Tell the CampusFit story live" description="Every control mutates deterministic synthetic data and updates relevant aggregates without a page reload." actions={<Button variant="danger" onClick={() => setResetConfirmationOpen(true)}><RotateCcw />Reset {state.university.shortName} demo</Button>} />
    ...
    <section className="demo-control-section"><ManagementSectionHeader eyebrow="Visit lifecycle" title="Plans and check-ins" description="Create or move synthetic visits through existing CampusFit lifecycle states." /><div className="demo-action-grid">
      ...
      <DemoAction icon={<Clock3 />} title="Delay a planned visit" body="Moves the next plan 20 minutes and shifts demand intervals." label="Delay 20 min" disabled={!hasPlannedVisit} onClick={() => action(delayNextPlannedVisit, 'Plan delayed — both forecast intervals recalculated')} />
      <DemoAction icon={<Power />} title="Check a user out" body="Removes one synthetic user from live counts and equipment pressure." label="Check user out" disabled={!hasSyntheticActiveVisit} onClick={() => action((current) => checkOutSyntheticVisit(current, tenant), 'Synthetic user checked out — live demand decreased')} />
      <DemoAction icon={<ArrowRightLeft />} title="Move an active visit" body="Moves one synthetic visit to the next facility." label="Move user" disabled={!hasSyntheticActiveVisit} onClick={() => action((current) => moveSyntheticVisit(current, tenant), 'Synthetic visit moved — both facilities updated')} />
    </div></section>
    ...
    <ConfirmationDialog open={resetConfirmationOpen} title={`Reset the ${state.university.shortName} demo?`} description="This replaces all current synthetic scenario changes with the original deterministic demo state." confirmLabel={`Reset ${state.university.shortName} demo`} tone="danger" onClose={() => setResetConfirmationOpen(false)} onConfirm={() => { resetTenant(tenant); setResetConfirmationOpen(false); }} />
  </div>;
}
