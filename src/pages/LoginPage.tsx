...
import { FormMessage, LoadingSkeleton } from '../components/ui';
...
export function LoginPage({ audience }: { audience: 'student' | 'staff' }) {
  ...
  const { accounts, accountsLoading, backendStatus, signInAs } = useCampusFit();
  ...
  const handleLogin = async (userId: string) => {
    ...
    try {
      await signInAs(tenant, user);
      navigate(`/${tenant}/${defaultRouteForRole(user.role)}`);
    } catch (error) {
      setLoginError(error instanceof Error ? error.message : 'CampusFit could not start this session.');
    } finally {
  ...
  return <main id="main-content" className={`login-page ${staffPortal ? 'login-page--staff' : ''}`} tabIndex={-1} style={{ '--tenant-primary': university.primaryColor, '--tenant-secondary': university.secondaryColor, '--tenant-accent': university.accentColor } as React.CSSProperties}>
    ...
    <div className="login-accounts"><Link className="back-link" to="/"><ArrowLeft size={17} /> Back to CampusFit</Link><div className="login-heading"><span className="data-label">{staffPortal ? 'Staff demonstration login' : 'Student demonstration login'}</span><h2>{staffPortal ? 'Authorized staff accounts' : 'Choose a student account'}</h2><p>No password is required in this prototype. Real {university.shortName} authentication is not connected.</p><small role="status">{accountsLoading ? 'Connecting to the CampusFit backend…' : backendStatus === 'connected' ? 'Backend session and tenant data ready.' : 'Local deterministic demo available.'}</small></div>{accountsLoading ? <LoadingSkeleton rows={3} label="Loading demo accounts" /> : <div className="account-list">{eligibleAccounts.map((account) => <button key={account.id} onClick={() => void handleLogin(account.id)} className="account-card" disabled={Boolean(pendingUserId)} aria-busy={pendingUserId === account.id}><span className="account-avatar">{initials(account.fullName)}</span><span><strong>{account.fullName}</strong><small>{pendingUserId === account.id ? 'Starting secure demo session…' : `${roleLabels[account.role]} · ${account.email}`}</small></span><ArrowRight size={18} /></button>)}</div>}{loginError ? <FormMessage tone="error">{loginError}</FormMessage> : null}<Link className="portal-switch" to={`/${tenant}/${staffPortal ? 'login' : 'staff-login'}`}>{staffPortal ? <UsersRound /> : <ShieldCheck />}<span><strong>{staffPortal ? 'Student access' : 'Recreation staff access'}</strong><small>Open the separate {staffPortal ? 'student experience' : 'staff and administration portal'}.</small></span><ArrowRight /></Link><div className="login-disclaimer"><LockKeyhole size={18} /><p><strong>{staffPortal ? 'Security model' : 'Demo environment'}</strong><br />{staffPortal ? 'This UI guard demonstrates role separation. Production authorization must also be enforced by university SSO and database RLS.' : 'Every person, plan, visit, forecast, and live count shown here is synthetic.'}</p></div></div>
  </main>;
}
