...
export function HomePage() {
  ...
  return <div className="page-stack home-page">
    ...
    <Modal open={checkInOpen} onClose={closeCheckIn} title={checkInStep === 4 ? 'You’re checked in' : 'I’m here'} description={checkInStep === 4 ? undefined : 'Manual facility selection works without location access.'} label={checkInStep === 4 ? 'Visit started' : `Quick check-in · ${Math.min(checkInStep + 1, 4)} of 4`}>
      ...
      {checkInStep === 1 ? <div className="checkin-step"><h3>What are you here for?</h3><SegmentedControl label="Visit purpose" value={visitIntent} onChange={handleIntentChange} options={[{ value: 'workout', label: 'Workout' }, { value: 'activity', label: 'Activity only' }]} />{visitIntent === 'activity' ? <label>Activity<select value={activity} onChange={(event) => handleActivityChange(event.target.value)}>{activities.filter((item) => state.facilities.some((facility) => facility.activities.includes(item.key))).map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}</select></label> : <label>Workout focus<select value={focus} onChange={(event) => setFocus(event.target.value)}>{workoutFocuses.map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}</select></label>}<div className="sheet-step-actions"><Button variant="ghost" onClick={() => setCheckInStep(0)}>Back</Button><Button disabled={visitIntent === 'activity' && !activity} onClick={() => setCheckInStep(2)}>Continue <ArrowRight /></Button></div></div> : null}
      ...
      {checkInStep === 3 ? <div className="checkin-step checkin-review"><h3>Ready to check in?</h3><div className="checkin-review-card"><span>{state.facilities.find((facility) => facility.id === facilityId)?.shortName}</span><strong>{visitIntent === 'activity' ? activities.find((item) => item.key === activity)?.label : workoutFocuses.find((item) => item.key === focus)?.label}</strong><small>{duration} minutes · CampusFit aggregate</small></div><label>Privacy<select value={privacy} onChange={(event) => setPrivacy(event.target.value as PrivacyLevel)}><option value="anonymous_aggregate">Anonymous aggregate</option><option value="friends_only">Friends only</option><option value="private">Private</option></select></label><div className="privacy-inline"><ShieldCheck /><p>Your name and exact visit details are never shown publicly.</p></div><div className="sheet-step-actions"><Button variant="ghost" onClick={() => setCheckInStep(2)}>Back</Button><Button size="large" onClick={handleSpontaneous}>{privacy === 'anonymous_aggregate' ? 'Check in anonymously' : 'Check in'} <ArrowRight /></Button></div></div> : null}
      {checkInStep === 4 ? <div className="checkin-confirmation" role="status"><span><CheckCircle2 /></span><h3>You’re checked in at {state.facilities.find((facility) => facility.id === facilityId)?.shortName}.</h3><p>Your visit is contributing anonymously to live gym insights.</p><Button size="large" onClick={closeCheckIn}>View active visit</Button></div> : null}
    </Modal>
