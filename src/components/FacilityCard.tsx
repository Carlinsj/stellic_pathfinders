import { ArrowUpRight, CheckCircle2, Clock3, Dumbbell, MapPin, UsersRound } from 'lucide-react';
...
export function FacilityCard({ state, facility, tenant, compact = false }: { state: DemoState; facility: Facility; tenant: string; compact?: boolean }) {
  ...
  const isClosed = isSpecialClosureActive(facility, state.now);
  ...
  return <article className={`facility-card ${compact ? 'facility-card--compact' : ''}`}>
    <div className="facility-card__top"><div className="facility-monogram" aria-hidden="true">{facility.shortName.slice(0, 2).toUpperCase()}</div><span className={`open-state ${isClosed ? 'open-state--closed' : ''}`}><CheckCircle2 />{isClosed ? 'Temporarily closed' : 'Open now'}</span></div>
    <div className="facility-card__content"><p className="eyebrow"><MapPin size={13} />{facility.travelMinutes} min away</p><h3>{facility.shortName}</h3>{compact ? null : <p>{facility.description}</p>}<div className="facility-card__status"><StatusPill level={forecast.crowdLevel}>{crowdLabel(forecast.crowdLevel)}</StatusPill><span><UsersRound />{aggregate.campusFitCheckIns} CampusFit users checked in</span></div></div>
    ...
    <DataSourceLabel>CampusFit prediction · {forecast.confidence} confidence · {forecast.expectedRange[0]}–{forecast.expectedRange[1]} range</DataSourceLabel>
    <Link className="card-link" to={`/${tenant}/facilities/${facility.id}`}>See gym details <ArrowUpRight size={17} /></Link>
  </article>;
}
